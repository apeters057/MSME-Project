...
class Motors(_Basic_class):
    ...
    MOTOR_2_PWM_PIN = "P12"
    MOTOR_2_DIR_PIN = "D5"
    VoltageRead_PWM_PIN = "P3"
    VoltageRead_DIR_PIN = "D0"
    ...
    def __init__(self, db=config_file, *args, **kwargs):
        ...
        self.voltage_id = int(self.db.get("voltage", default_value=0))
        ...
        self.motors = [
            ...
            Motor(PWM(self.VoltageRead_PWM_PIN), Pin(self.VoltageRead_DIR_PIN))
    ...
    def voltage(self):
        """voltage motor"""
        if self.left_id not in range(1, 4):
            raise ValueError(
                "left motor is not set yet, set it with set_left_id(1/2)")
        return self.motors[self.voltage_id-1]
    ...
    def set_voltage_id(self, id):
        ...
        if id not in range(1, 4):
            raise ValueError("Voltage id must be 3")
        self.right_id = id
        self.db.set("right", id)
