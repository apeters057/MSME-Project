...
baseFileName = '106_UnitStep_PWM50_Test';
...
title('Avg Motor Voltage @ 50% PWM  (10 uF)', 'FontWeight', 'bold');
xlabel('Avg Time [s]')
ylabel('Avg Voltage [V]')
subplot(2, 1, 2);
plot(t_average,accelX_driving_average)
title('Avg. Foward Car Acceleration @ 50% PWM  (10 uF)', 'FontWeight', 'bold');
xlabel('Avg. Time [s]')
ylabel('Avg. Acceleration [m/s^2]')

%% Formulate T(s) , V/PWM --> (Vss) / (Ts + 1)

%Interpolate the Time constant at which it takes for the Motor to reach 63%
%of its steady state value

voltage_tau = 0.632*max(motor_voltage_driving_average);
tau = interp1(motor_voltage_driving_average,t_average,voltage_tau,'linear'); %[s]
disp(tau)

num = [0 max(motor_voltage_driving_average)];
denum = [tau 1];
plant = tf(num,denum);
figure(2);step(plant)
[sys_response , sim_time ] = step(plant);

index=find(t_average < max(sim_time));
t_avg_plot = t_average(index);
voltage_plot = motor_voltage_driving_average(index);

figure(3);
title('Real System vs Simulated System', 'FontWeight', 'bold');
plot(sim_time, sys_response,'b-', 'LineWidth', 2);
hold on;
plot(t_avg_plot,voltage_plot, 'r--', 'LineWidth', 1.5);
hold on;
legend('Simulated System', 'Real System');
xlabel('Time [s]')
ylabel('Voltage [V]')
hold off;
