...
Ki = 0.0003
Kd = 0.02
deadband = 5  # Deadband to ignore small errors

# Steering parameters
STEER_CENTER = -5  # Default steering center angle
STEER_LIMIT = 28  # Maximum allowable steering angle
...
base_speed = 30  # Base speed for the car
...
def find_green_line_center(frame):
    ...
    # Convert frame to HSV and create a mask for green
    ...
    cv2.circle(frame, green_center, 7, (0, 255, 0), -1)
...
def real_time_velocity_plotting(mean_tau, mean_alpha, shared_velocity):
    ...
    while True:
        try:
            ...
            new_x_hat = np.exp(A * T) * x_hat[-1] + H * u + 0.005 * (y - (A * x_hat[-1] + B * u))
...
def lane_centering(shared_velocity):
    ...
    # Speed parameters
    initial_boost_duration = .3  # Boost duration in seconds
    boosted_speed = 40 # Speed during boost
    stable_speed = 35.5  # Speed after stabilization
    ramp_up_increment = 7  # Speed increment for ramp-up
    ...
    while True:
        ...
        # PID control an ane_center_y = lane_center
        ...
        # Calculate lane deviation and adjust speed
        lane_deviation = abs(lane_center_x - FRAME_CENTER_X) / (FRAME_WIDTH/2) * 100
