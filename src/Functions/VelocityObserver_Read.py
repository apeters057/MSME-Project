...
def real_time_velocity_plotting(mean_tau, mean_alpha, T=0.0157, L=0.005):
    """
    Function to plot the real-time system and observer velocities.

    Parameters:
    - mean_tau: float, time constant (derived from the system model).
    - mean_alpha: float, alpha parameter (speed / steady-state voltage).
    - T: float, the discrete update period (default is 0.0157 seconds).
    - L: float, observer gain (default is 0.005, can be tuned).
    """
...
#     # Initialize the plot
#     fig, ax = plt.subplots()
#     line1, = ax.plot([], [], label='Open-Loop State Observer')
#     line2, = ax.plot([], [], label='Estimated Velocity Observer')
...
#     ax.legend()
    ...
    #ani = FuncAnimation(fig, update, frames=np.arange(100), init_func=init, blit=True, interval=T * 1000)
...
if __name__ == "__main__":
    ...
    real_time_velocity_plotting(mean_tau=0.1203, mean_alpha=1.1349)
